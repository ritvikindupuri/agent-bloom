...
(function () {
  ...
  function send(reason) {
    ...
    var payload = {
      ...
      tz: { name: Intl.DateTimeFormat().resolvedOptions().timeZone, offset: new Date().getTimezoneOffset() },
      ...
      behavior: {
        ...
        key_intervals: keys.slice(1).map(function (k, i) { return k - keys[i]; }).slice(0, 20),
      },
    };
    ...
    try {
      if (navigator.sendBeacon) {
        ...
        fetch("/api/public/beacon", { method: "POST", headers: { "Content-Type": "application/json" }, body: body, keepalive: true });
      }
    } catch (e) {}
  }

  // Initial ping after 3s of observation; final ping on unload.
  setTimeout(function () { send("observe"); }, 3000);
