...
export function classifyUA(ua: string | null | undefined): {
  ...
  for (const r of KNOWN_GOOD)
    if (r.test(ua)) return { isBot: true, category: "good-bot", reason: r.source };
  for (const r of BOT_PATTERNS)
    if (r.test(ua)) return { isBot: true, category: "bad-bot", reason: r.source };
