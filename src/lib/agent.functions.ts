...
const tools: any[] = [
  {
    ...
    function: {
      name: "search_logs",
      description: "Run an Elasticsearch DSL query against the user's log index. Returns aggregations and a sample of hits.",
      parameters: {
        type: "object",
        properties: {
          range_minutes: { type: "number", description: "Time window in minutes, e.g. 60 for last hour. Max 43200 (30 days).", default: 60 },
          query_filter: { type: "object", description: "Optional ES bool/query clauses to add (e.g. { term: { 'client.ip': '1.2.3.4' } })", additionalProperties: true },
          aggregation: {
            type: "string",
            enum: ["top_user_agents", "top_ips", "top_paths", "status_codes", "requests_per_minute", "ip_user_agents"],
            description: "Which aggregation to compute.",
          },
          size: { type: "number", description: "How many buckets to return (default 15, max 50).", default: 15 },
          focus_ip: { type: "string", description: "Required when aggregation=ip_user_agents — which IP to break down." },
        },
  ...
  },
  {
    ...
    function: {
      ...
      description: "Fetch a small sample of raw log documents matching a filter, to inspect details.",
      parameters: {
        type: "object",
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      parameters: {
        type: "object",
        properties: {
          kind: { type: "string", description: "e.g. 'scraper', 'credential-stuffing', 'scanner', 'fake-browser'" },
...
async function runTool(name: string, args: any, ctx: { supabase: any; userId: string; conn: any; connectionId: string }) {
  ...
  if (name === "search_logs") {
    ...
    const aggMap: Record<string, any> = {
      ...
      requests_per_minute: { date_histogram: { field: tsField, fixed_interval: "1m", min_doc_count: 0 } },
      ip_user_agents: {
    ...
    const body = { size: 0, query: { bool: { filter: filters } }, aggs: { result: aggMap[args.aggregation] } };
    const res: any = await esSearch(auth, conn.index_pattern, body).catch((e: Error) => ({ __error: e.message }));
    ...
    const annotated = buckets.map((b: any) => {
      ...
      if (args.aggregation === "top_user_agents") {
        ...
        out.bot = c.isBot; out.category = c.category; out.reason = c.reason;
      }
      if (args.aggregation === "ip_user_agents") {
        out.user_agents = (b.uas?.buckets ?? []).map((u: any) => ({
          ua: u.key, count: u.doc_count, ...classifyUA(u.key),
        }));
      }
      return out;
    });
    return { aggregation: args.aggregation, total_hits: res?.hits?.total?.value ?? 0, buckets: annotated };
  }
  ...
  if (name === "record_threat") {
    const { error } = await supabase.from("threat_findings").insert({
      ...
      kind: args.kind, severity: args.severity, title: args.title, summary: args.summary,
  ...
  .handler(async ({ data, context }) => {
    ...
    for (const m of (prior ?? [])) {
      if (m.role === "user" || m.role === "assistant") {
        ...
        chatMessages.push({ role: "tool", name: m.tool_name, content: JSON.stringify(m.tool_result ?? {}), tool_call_id: (m.tool_calls as any)?.id ?? "t" });
      }
    }
    ...
    for (let round = 0; round < 6; round++) {
      ...
      if (!res.ok) {
        ...
        if (res.status === 429) throw new Error("Rate limit reached on Lovable AI. Try again in a minute.");
      ...
      if (toolCalls.length === 0) {
        ...
        await supabase.from("agent_messages").insert({
          conversation_id: conversationId, user_id: userId, role: "assistant", content: finalContent,
        });
      ...
      for (const call of toolCalls) {
        ...
        const result = await runTool(call.function.name, args, { supabase, userId, conn, connectionId: conn.id });
        traces.push({ tool: call.function.name, args, result });
        await supabase.from("agent_messages").insert({
          conversation_id: conversationId, user_id: userId, role: "tool",
          tool_name: call.function.name, tool_calls: { id: call.id }, tool_result: result, content: null,
        });
        chatMessages.push({ role: "tool", tool_call_id: call.id, name: call.function.name, content: JSON.stringify(result).slice(0, 18000) });
      }
    }
...
export const deleteConversation = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
    const { error } = await supabase.from("agent_conversations").delete().eq("id", data.id).eq("user_id", userId);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
