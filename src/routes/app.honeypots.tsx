  ...
  return (
    <div className="p-10 max-w-5xl">
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Trap URLs that look like real pages. Any visitor is, by definition, a bot — Chaff
            captures their fingerprint live.
          </p>
        </div>
      </div>
      ...
      <div className="mt-6 flex gap-2">
        <Input
          placeholder="Label (e.g. fake-pricing)"
          value={label}
      ...
      <div className="mt-8 rounded-lg border border-border bg-surface/40 divide-y divide-border">
        {q.data?.honeypots?.length ? (
          q.data.honeypots.map((h: any) => {
              ...
              <div key={h.id} className="p-5">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => mDelete.mutate(h.id)}
                      className="rounded-md border border-border px-2 py-1 text-xs hover:bg-destructive/10 hover:text-destructive"
                    >
  ...
  return (
    <div>
      ...
      <div className="flex gap-1.5">
        <code
          className={`flex-1 rounded-md border border-border bg-background px-2.5 py-1.5 text-xs font-mono ${multi ? "whitespace-pre" : "truncate"}`}
        >
          {value}
        </code>
        <button
