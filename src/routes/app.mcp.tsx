...
function McpPage() {
  ...
  const m = useMutation({
    ...
    onSuccess: (d) => {
      setFresh(d.token);
      setLabel("");
      qc.invalidateQueries({ queryKey: ["mcpTokens"] });
    },
  ...
  return (
    <div className="p-10 max-w-4xl">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Give Claude, Cursor, ChatGPT — any MCP-aware agent — secure access to your Chaff bot
        intelligence.
      </p>
      ...
      <div className="mt-6 rounded-lg border border-border bg-surface/40 p-5">
        ...
        <div className="flex gap-2">
          <code className="flex-1 rounded-md border border-border bg-background px-2.5 py-1.5 text-xs font-mono">
            {mcpUrl}
          </code>
          <Hint label="Copy MCP endpoint URL">
            <button
              onClick={() => {
        ...
        <div className="mt-4 text-xs text-muted-foreground">
          Tools exposed: <code className="text-foreground">is_known_bot</code> ·{" "}
          <code className="text-foreground">list_recent_campaigns</code> ·{" "}
          <code className="text-foreground">get_campaign</code> ·{" "}
          <code className="text-foreground">lookup_fingerprint</code>
        </div>
      </div>
      ...
      {fresh && (
        <div className="mt-4 rounded-lg border border-primary/40 bg-primary/5 p-4">
          ...
          <div className="flex gap-2">
            <code className="flex-1 rounded-md border border-border bg-background px-2.5 py-1.5 text-xs font-mono break-all">
              {fresh}
            </code>
            <button
              onClick={() => {
        ...
        {q.data?.tokens?.length ? (
          q.data.tokens.map((t: any) => (
            <div key={t.id} className="p-4 flex items-center justify-between">
              ...
              {!t.revoked_at && (
                <Hint label="Permanently disable this token. MCP clients using it will lose access immediately.">
                  <button
                    onClick={() => r.mutate(t.id)}
