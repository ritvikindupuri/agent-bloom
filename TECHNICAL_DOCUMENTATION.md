...
3. [Detailed Feature & Agent Breakdown](#detailed-feature--agent-breakdown)
   ...
   - [Campaigns](#campaigns)
   - [Harvest (Intel Gathering)](#harvest-intel-gathering)
...
### Harvest (Intel Gathering)
- **Purpose**: Enriches threat data by gathering external intelligence on suspicious IPs.
- **Capabilities (`app.harvest.tsx`)**: Integrates IP intelligence tools. When a suspicious IP is found, Harvest queries external databases to check if the IP belongs to a known VPN, Tor exit node, or cloud provider (AWS/DigitalOcean), providing critical context to the SecOps user.
